...
function classificarQuartasDeFinal() {
    ...
    if (!$conn->query("TRUNCATE TABLE quartas_de_final")) {
        echo "Erro ao truncar tabela quartas_de_final: " . $conn->error;
        return;
    }
    if (!$conn->query("TRUNCATE TABLE quartas_de_final_confrontos")) {
        echo "Erro ao truncar tabela quartas_de_final_confrontos: " . $conn->error;
        return;
    }
    ...
    if ($fase_final == 'oitavas') {
        ...
        foreach ($confrontos as $confronto) {
            ...
            $stmt->bind_param("isis", $confronto['timeA']['id'], $confronto['timeA']['nome'], $confronto['timeB']['id'], $confronto['timeB']['nome']);
            $stmt->execute();
            $stmt->close();
        }
...
function classificarSemifinais() {
    ...
    // Limpar as tabelas de semifinais e confrontos
    ...
    // Obter os vencedores dos confrontos das quartas de final
    $result = $conn->query("
        SELECT 
    ...
    foreach ($confrontos as $confronto) {
        ...
        $stmt = $conn->prepare("INSERT INTO semifinais_confrontos (timeA_id, timeA_nome, timeB_id, timeB_nome, fase) VALUES (?, ?, ?, ?, 'semifinais')");
        $stmt->bind_param("isis", $timeA['time_id'], $timeA['time_nome'], $timeB['time_id'], $timeB['time_nome']);
        $stmt->execute();
        $stmt->close();
    }
...
function classificarFinal() {
    ...
    $times = [];
    while ($row = $result->fetch_assoc()) {
        $time_classificado_id = $row['time_classificado_id'];
        if ($time_classificado_id) {
            $time_result = $conn->query("SELECT * FROM semifinais WHERE time_id = $time_classificado_id")->fetch_assoc();
            if ($time_result) {
                $times[] = $time_result;
            } else {
                echo "Erro ao encontrar informações do time classificado: $time_classificado_id";
            }
        }
    }
    ...
    $stmt = $conn->prepare("INSERT INTO final_confrontos (timeA_id, timeA_nome, timeB_id, timeB_nome, fase) VALUES (?, ?, ?, ?, 'final')");
    $stmt->bind_param("isis", $timeA['time_id'], $timeA['time_nome'], $timeB['time_id'], $timeB['time_nome']);
    if ($stmt->execute()) {
